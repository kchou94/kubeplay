...
common::install_tools(){
  ...
  # Install helm
  local helm_tar_file
  helm_tar_file=$(find "${RESOURCES_NGINX_DIR}"/files -type f -name "helm*-linux-${ARCH}.tar.gz" | sort -r --version-sort | head -n1)
  ...
  local nerdctl_tar_file
  nerdctl_tar_file=$(find "${RESOURCES_NGINX_DIR}"/tools -type f -name "nerdctl-full-*-linux-${ARCH}.tar.gz" | sort -r --version-sort | head -n1)
...
common::rudder_config(){
  ...
  nginx_http_port="${NGINX_HTTP_PORT}:8080" yq eval --inplace '.services.nginx.ports[1] = strenv(nginx_http_port)' "${COMPOSE_YAML_FILE}"
  registry_https_port="${REGISTRY_HTTPS_PORT}:443" yq eval --inplace '.services.nginx.ports[0] = strenv(registry_https_port)' "${COMPOSE_YAML_FILE}"
...
common::generate_domain_certs(){
  if [[ ${GENERATE_DOMAIN_CRT} == "true" ]]; then
    local DOMAIN
    DOMAIN=$(echo "${REGISTRY_DOMAIN}" | sed 's/[^.]*./*./')
...
common::load_images(){
  ...
  local IMAGES
  IMAGES=$(find "${IMAGES_DIR}" -type f -name '*.tar')
